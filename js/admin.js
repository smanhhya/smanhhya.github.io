...
// --- محرك بناء الواجهات الذاتي (يضمن عدم ضياع أي خانة) ---
function buildDashboardUI() {
    ...
    if (pStore) pStore.innerHTML = `
        <div class="bg-white border rounded-xl p-4 shadow-sm space-y-3 mb-4">
            <h3 class="font-black text-brand-navy"><i class="fa-solid fa-store text-brand-cyanDark"></i> بيانات المتجر الأساسية</h3>
            ...
            <div><label class="text-xs font-bold text-gray-500">وصف المتجر (يظهر في الهيدر أسفل الاسم)</label><textarea id="admin-store-desc" rows="2" class="w-full border rounded-lg p-2 font-bold text-brand-navy"></textarea></div>
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm space-y-3">
            ...
            <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    <div><label class="text-[9px] text-gray-500">العنوان الرئيسي</label><input type="text" id="admin-badge-${i}-title" class="w-full border p-1.5 rounded text-xs font-bold text-brand-navy"></div>
                    <div><label class="text-[9px] text-gray-500">الوصف الفرعي</label><input type="text" id="admin-badge-${i}-desc" class="w-full border p-1.5 rounded text-xs font-bold text-brand-navy"></div>
                </div>
            </div>`).join('')}
        </div>
    `;
    ...
    if (pProducts) pProducts.innerHTML = `
        <div class="flex justify-between items-center bg-brand-light/30 p-3 rounded-lg border border-brand-cyan/20">
            ...
            <button onclick="addNewAdminProduct()" class="bg-brand-cyanDark text-white text-xs font-bold px-3 py-1.5 rounded shadow-sm">+ منتج جديد</button>
        </div>
        <div id="admin-inputs-container" class="space-y-4 mt-4"></div>
    `;

    const pOrders = document.getElementById('admin-panel-orders');
    if (pOrders) pOrders.innerHTML = `
        <input type="text" id="order-search" placeholder="ابحث باسم العميل أو رقم الموبايل..." class="w-full border-2 border-gray-200 rounded-xl p-3 mb-4 font-bold text-brand-navy outline-none" onkeyup="renderOrdersList()">
    ...
    if (pDispatch) pDispatch.innerHTML = `
        <div class="bg-white border rounded-xl p-4 shadow-sm mb-4">
            ...
            <div class="mb-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <input type="text" id="dispatch-driver-phone" class="w-full border rounded-lg p-2 font-bold text-brand-navy text-left" dir="ltr" placeholder="أو اكتب رقم واتساب المندوب يدوياً">
            </div>
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm border-l-4 border-l-blue-500">
            ...
            <button onclick="addNewAdminDriver()" class="w-full bg-gray-50 text-brand-navy text-sm font-bold py-2 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors"><i class="fa-solid fa-plus"></i> إضافة مندوب جديد</button>
        </div>
    `;

    const pDelivery = document.getElementById('admin-panel-delivery');
    if (pDelivery) pDelivery.innerHTML = `
        <div class="bg-white border rounded-xl p-4 shadow-sm mb-4">
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm">
            ...
            <button onclick="addNewAdminZone()" class="w-full bg-gray-50 text-brand-navy text-sm font-bold py-2 rounded-lg border border-gray-200"><i class="fa-solid fa-plus"></i> إضافة منطقة جديدة</button>
        </div>
    `;

    const pTexts = document.getElementById('admin-panel-texts');
    if (pTexts) pTexts.innerHTML = `
        <div class="bg-white border rounded-xl p-4 shadow-sm">
    ...
    if (pMarketing) pMarketing.innerHTML = `
            ...
            <div class="grid grid-cols-2 gap-2 bg-green-50 p-3 rounded-lg border border-green-100 mb-3">
                <div>
                    ...
                    <select id="admin-reward-type" class="w-full border rounded p-2 font-bold text-brand-navy" onchange="document.getElementById('reward-max-disc-container').classList.toggle('hidden', this.value !== 'percent')">
                        <option value="fixed">مبلغ ثابت (ج.م)</option><option value="percent">نسبة مئوية (%)</option><option value="free_delivery">توصيل مجاني</option>
                    </select>
                </div>
                <div><label class="text-[10px] font-bold text-gray-500 block mb-1">قيمة الخصم</label><input type="number" id="admin-reward-value" class="w-full border rounded p-2 font-bold text-brand-navy text-center"></div>
                <div id="reward-max-disc-container" class="col-span-2 hidden">
                    ...
                    <input type="number" id="admin-reward-max-discount" class="w-full border rounded p-2 font-bold text-brand-navy text-center" placeholder="سيبه 0 لو مفيش حد أقصى">
                </div>
                <div class="col-span-2"><label class="text-[10px] font-bold text-gray-500 block mb-1">عدد الأكواد اللي السيستم يولدها</label><input type="number" id="admin-reward-max-generations" class="w-full border rounded p-2 font-bold text-brand-navy text-center" placeholder="سيبه 0 لعدد لا نهائي ∞"></div>
            </div>
            <div>
                <label class="text-[10px] font-bold text-gray-500 block mb-1">رسالة العميل (التي تظهر مع الكود)</label>
                <textarea id="admin-loyalty-msg" rows="2" class="w-full border rounded-lg p-2 text-xs font-bold text-brand-navy"></textarea>
            </div>
        </div>
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm mb-4 border-l-4 border-l-blue-500">
            ...
            <div><label class="text-[10px] font-bold text-gray-500 block mb-1">الأسماء (افصل بينها بفاصلة ،)</label><textarea id="admin-noti-names" rows="2" class="w-full border rounded-lg p-2 text-xs font-bold"></textarea></div>
            <div class="mt-2"><label class="text-[10px] font-bold text-gray-500 block mb-1">المناطق (افصل بينها بفاصلة ،)</label><textarea id="admin-noti-places" rows="2" class="w-full border rounded-lg p-2 text-xs font-bold"></textarea></div>
        </div>
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm mb-4 border-l-4 border-l-yellow-500">
            ...
            <label class="flex items-center gap-2 mb-2"><input type="checkbox" id="admin-banner-active" class="accent-yellow-500 w-4 h-4"> <span class="text-sm font-bold text-yellow-700">تفعيل الشريط العلوي</span></label>
            <div><label class="text-[10px] font-bold text-gray-500 block mb-1">الرسائل (كل رسالة في سطر منفصل)</label><textarea id="admin-marquee-messages" rows="3" class="w-full border rounded-lg p-2 text-xs font-bold"></textarea></div>
        </div>
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm">
            <div class="flex justify-between items-center mb-3">
                ...
                <button onclick="addNewPromoCode()" class="bg-gray-800 hover:bg-black text-white text-xs px-3 py-1.5 rounded shadow-sm">+ كود ثابت جديد</button>
            </div>
            <label class="flex items-center gap-2 mb-4 bg-gray-50 p-2 rounded border"><input type="checkbox" id="admin-show-promo-field" class="accent-brand-cyanDark w-4 h-4"> <span class="text-xs font-bold">إظهار خانة الكوبون بالسلة</span></label>
            <div id="admin-promos-container" class="space-y-3"></div>
        </div>
    `;

    const pAdvanced = document.getElementById('admin-panel-advanced');
    if (pAdvanced) pAdvanced.innerHTML = `
        <div class="bg-white border rounded-xl p-4 shadow-sm mb-4 border-l-4 border-l-brand-cyanDark">
            <h3 class="font-black text-brand-navy mb-3"><i class="fa-solid fa-language text-brand-cyanDark"></i> قوالب رسائل النظام الذكية</h3>
            <div class="space-y-4">
                <div><label class="text-xs font-bold text-gray-500 block mb-1">رسالة الواتساب للعميل (بعد الطلب)</label><textarea id="admin-whatsapp-template" rows="4" class="w-full border rounded-lg p-2 text-xs font-bold"></textarea></div>
                <div><label class="text-xs font-bold text-gray-500 block mb-1">رسالة الإرسال للمندوب (التوزيع)</label><textarea id="admin-dispatch-template" rows="4" class="w-full border rounded-lg p-2 text-xs font-bold"></textarea></div>
                ...
                <div><label class="text-xs font-bold text-gray-500 block mb-1">هاشتاج الدفعة الحالية</label><input type="text" id="admin-batch-hashtag" class="w-full border rounded-lg p-2 text-xs font-bold"></div>
            </div>
        </div>
        <div class="bg-red-50 border border-red-200 rounded-xl p-4 shadow-sm mt-4">
            ...
            <div class="space-y-3">
                <button onclick="resetStatsOnly()" class="w-full bg-orange-500 text-white font-bold py-3 rounded-lg hover:bg-orange-600 shadow-md">تصفير الإحصائيات (مبيعات وطلبات)</button>
                <button onclick="deleteAllOrders()" class="w-full bg-red-600 text-white font-bold py-3 rounded-lg hover:bg-red-700 shadow-md">مسح جميع سجلات الطلبات نهائياً</button>
            </div>
        </div>
    `;

    const pGallery = document.getElementById('admin-panel-gallery');
    if (pGallery) pGallery.innerHTML = `
    ...
    if (pTheme) pTheme.innerHTML = `
        <div class="bg-white border rounded-xl p-4 shadow-sm border-l-4 border-l-brand-cyanDark">
            <h3 class="font-black text-brand-navy mb-3"><i class="fa-solid fa-palette text-brand-cyanDark"></i> ألوان الموقع (تغيير فوري)</h3>
            <div class="grid grid-cols-1 gap-4 mb-4">
                <div class="flex items-center justify-between bg-gray-50 p-2 rounded-lg border">
                    <label class="text-xs text-gray-700 font-bold">اللون الأساسي (أخضر سمان ههيا)</label>
                ...
                <div class="flex items-center justify-between bg-gray-50 p-2 rounded-lg border">
                    <label class="text-xs text-gray-700 font-bold">لون التميز والخصومات (الأحمر)</label>
                ...
                <div class="flex items-center justify-between bg-gray-50 p-2 rounded-lg border">
                    <label class="text-xs text-gray-700 font-bold">لون التنبيهات والأيقونات (الأصفر)</label>
...
// --- نظام الدخول الشامل للمتغيرات ---
window.openAdminDashboard = () => {
    buildDashboardUI(); // 1. نبني الهيكل الأول لضمان وجود كل الـ IDs
    ...
    if(textsCont && typeof textsConfig !== 'undefined') {
        ...
        textsConfig.forEach(t => {
            ...
            textsCont.innerHTML += `<div><label class="text-[10px] font-bold text-gray-500 block mb-1">${t.label}</label><input type="text" id="ui-txt-${t.id}" value="${val}" class="w-full border rounded p-2 text-xs font-bold text-brand-navy"></div>`;
        });
    }

    // 3. تشغيل وظائف الرسم الداخلي
    renderAdminDrivers();
    renderAdminZones();
    renderAdminPromos();
    renderGalleryTabUI();
        ...
        <div class="bg-white border border-gray-200 rounded-xl p-3 shadow-sm mb-3">
            ...
            <div id="edit-p-${id}" class="hidden mt-4 space-y-3 border-t pt-3">
                <div class="grid grid-cols-2 gap-2">
                    <div><label class="text-[10px] font-bold text-gray-500">الاسم</label><input type="text" id="p-name-${id}" value="${p.name}" class="w-full border p-2 rounded text-xs font-bold text-brand-navy"></div>
                    <div><label class="text-[10px] font-bold text-gray-500">الوزن / الوصف</label><input type="text" id="p-weight-${id}" value="${p.weight}" class="w-full border p-2 rounded text-xs font-bold text-brand-navy"></div>
                    <div><label class="text-[10px] font-bold text-gray-500">السعر (ج)</label><input type="number" id="p-price-${id}" value="${price}" class="w-full border p-2 rounded text-xs font-bold text-brand-cyanDark"></div>
                    <div><label class="text-[10px] font-bold text-gray-500">المخزون المتوفر</label><input type="number" id="p-stock-${id}" value="${stock}" class="w-full border p-2 rounded text-xs font-black ${stock<5?'border-red-500 bg-red-50 text-red-600':'text-brand-navy'}"></div>
                    <div class="col-span-2"><label class="text-[10px] font-bold text-gray-500">رابط الصورة</label><input type="text" id="p-img-${id}" value="${imgSrc}" class="w-full border p-2 rounded text-[10px] font-bold text-gray-500" dir="ltr"></div>
                </div>
                <div class="flex flex-wrap gap-3 bg-gray-50 p-2 rounded border mb-2">
                    ...
                    <input type="number" id="p-old-${id}" value="${oldPrice}" class="w-12 border rounded p-1 text-[10px] text-center bg-white text-red-500 font-bold line-through ${isDisc?'':'hidden'}">
                </div>
...
window.renderGalleryTabUI = () => {
    const c = document.getElementById('admin-gallery-list');
    if(!c) return;
...
window.renderAdminDrivers = () => {
    ...
    let optionsHtml = '<option value="">-- اكتب الرقم يدوياً --</option>';
    ...
    tempPromoCodes.forEach((p,i) => {
        ...
        <div class="flex flex-col gap-2 ${p.isAuto ? 'bg-blue-50/50' : 'bg-gray-50'} p-3 border border-gray-200 rounded-xl mb-3 relative overflow-hidden shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-2 mt-1">
                ...
                <div><label class="text-[10px] font-bold text-gray-500 block mb-0.5">أقصى خصم بالجنيه</label><input type="number" value="${p.maxDiscount}" placeholder="بدون" class="w-full border rounded p-1.5 text-xs text-center font-bold text-brand-navy outline-none ${p.type !== 'percent' ? 'bg-gray-100 opacity-50' : ''}" ${p.type !== 'percent' ? 'disabled' : ''} onchange="tempPromoCodes[${i}].maxDiscount=parseInt(this.value) || 0"></div>
            </div>
            <div class="grid grid-cols-2 gap-2 mt-1">
                <div><label class="text-[10px] font-bold text-gray-500 block mb-0.5">مخصص لرقم الموبايل</label><input type="text" value="${p.customerPhone || ''}" placeholder="أي شخص" class="w-full border rounded p-1.5 text-xs text-center font-bold text-brand-navy outline-none" onchange="tempPromoCodes[${i}].customerPhone=this.value" dir="ltr"></div>
                <div><label class="text-[10px] font-bold text-gray-500 block mb-0.5">يُستخدم كام مرة؟</label><input type="number" value="${p.usesLeft !== null ? p.usesLeft : ''}" placeholder="سيبه فاضي لعدد ∞" class="w-full border rounded p-1.5 text-xs text-center font-bold text-brand-navy outline-none placeholder-gray-400" onchange="tempPromoCodes[${i}].usesLeft=this.value === '' ? null : parseInt(this.value)"></div>
            </div>
