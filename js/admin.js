...
// --- ريندر الأكواد مع ميزة البحث واللانهاية ---
window.renderAdminPromos = () => { 
    ...
    if(query) {
        filteredPromos = tempPromoCodes.filter(p => p.code.toLowerCase().includes(query) || (p.customerPhone && p.customerPhone.includes(query)));
    }

    if(filteredPromos.length === 0) { c.innerHTML = '<p class="text-center text-gray-400 text-xs font-bold">لا يوجد أكواد مسجلة</p>'; }

    filteredPromos.forEach((p) => {
        let i = tempPromoCodes.indexOf(p); // الأصل عشان الحفظ
        ...
        <div class="flex flex-col gap-2 bg-gray-50 p-3 border border-gray-200 rounded-xl mb-3 relative overflow-hidden shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-2 mt-1">
                <div>
                    ...
                    <input type="number" value="${p.minOrder || 0}" placeholder="مثال: 150" class="w-full border rounded p-1.5 text-xs text-center font-bold text-brand-navy outline-none focus:border-brand-cyan" onchange="tempPromoCodes[${i}].minOrder=parseInt(this.value) || 0">
                </div>
                <div>
                    <label class="text-[10px] font-bold text-gray-500 block mb-0.5">الحد الأقصى للخصم (ج)</label>
                    <input type="number" value="${p.maxDiscount || 0}" placeholder="بدون حد" class="w-full border rounded p-1.5 text-xs text-center font-bold text-brand-navy outline-none focus:border-brand-cyan ${p.type !== 'percent' ? 'bg-gray-100 cursor-not-allowed opacity-50' : ''}" ${p.type !== 'percent' ? 'disabled' : ''} onchange="tempPromoCodes[${i}].maxDiscount=parseInt(this.value) || 0">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 mt-1">
                ...
                <div>
                    <label class="text-[10px] font-bold text-gray-500 block mb-0.5">عدد مرات الاستخدام</label>
                    <input type="number" value="${p.usesLeft === null ? '' : p.usesLeft}" placeholder="لا نهائي" class="w-full border rounded p-1.5 text-xs text-center font-bold text-brand-navy outline-none focus:border-brand-cyan" onchange="let val = this.value; tempPromoCodes[${i}].usesLeft = (val === '' || val <= 0) ? null : parseInt(val);">
                </div>
            </div>
...
// --- ريندر الطلبات مع البحث والتفاصيل الجديدة ---
window.renderOrdersList = () => {
    ...
    if (searchQuery) {
        filtered = filtered.filter(o => o.customerName.toLowerCase().includes(searchQuery) || o.customerPhone.includes(searchQuery));
    }
    ...
    filtered.forEach(order => {
        ...
        <div class="bg-white border rounded-xl p-4 shadow-sm relative overflow-hidden">
            ...
            ${order.usedPromo ? `<div class="text-[10px] text-red-500 font-bold mb-1 bg-red-50 p-1.5 rounded"><i class="fa-solid fa-ticket"></i> العميل استخدم كود: ${order.usedPromo}</div>` : ''}
