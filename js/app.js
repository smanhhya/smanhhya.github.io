...
function getAvailableStock(id) { 
    ...
    // قراءة دقيقة من مخزن الدفعة المفتوحة جوة الـ CRM
    ...
    db.collection('inventory').doc('batches').onSnapshot(doc => {
        if(doc.exists) {
            ...
            if(batchSelect && batchContainer) {
                ...
                batchSelect.innerHTML = ''; // مسح الطلب العام
            ...
            if(batchId) {
                ...
                if(docSnap.exists) {
                    ...
                    if(batchesData[batchId]) {
                        ...
                        for (let id in cart) { 
                            batchesData[batchId].booked[id] = (parseInt(batchesData[batchId].booked[id]) || 0) + cart[id].quantity; 
                        }
                        promises.push(batchRef.set(batchesData)); // جلب وحفظ آمن وشامل للملف
                    }
                }
            }
