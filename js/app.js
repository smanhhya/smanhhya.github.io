...
function getAvailableStock(id) { 
    ...
    if (batchId && globalBatches[batchId]) {
        ...
        // لو الـ CRM مش حاطط رقم للدفعة، اقرأ المخزن الأساسي
...
window.finalCheckoutStep = async function() {
    ...
    try {
        ...
        if (hasCloud && db) {
            ...
            if(batchId) {
                let bookedUpdates = {};
                for (let id in cart) { bookedUpdates[id] = firebase.firestore.FieldValue.increment(cart[id].quantity); }
                promises.push(db.collection("inventory").doc("batches").set({ [batchId]: { booked: bookedUpdates } }, { merge: true }));
            }
